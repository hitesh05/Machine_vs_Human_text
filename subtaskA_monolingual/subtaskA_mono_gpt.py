...
def prepare_data(text, max_length):
    # Ensure that text is a list of strings (batch or single pretokenized example)
    if not isinstance(text, list):
        text = [text]
    
    inputs = tokenizer(text, truncation=True, padding='max_length', max_length=max_length, return_tensors='pt', is_split_into_words=True)
...
for epoch in range(num_epochs):
    ...
    for text, label in progress_bar:
        input_ids, attention_mask = prepare_data(text, max_sequence_length)
