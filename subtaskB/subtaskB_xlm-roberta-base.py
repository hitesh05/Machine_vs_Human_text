...
train_data = pd.read_json('../SemEval8/SubtaskB/subtaskB_train.jsonl', lines=True)
# train_data = train_data.sample(n=10000)
dev_data = pd.read_json('../SemEval8/SubtaskB/subtaskB_dev.jsonl', lines=True)
...
for epoch in range(num_epochs):
    ...
    with open("output_subtaskB_multi_xlmrobertabase.txt", "a") as output_file:
      output_line = f"Epoch {epoch + 1}/{num_epochs} - Avg. Train Loss: {avg_train_loss:.4f} - Validation Accuracy: {accuracy:.4f}\n"
      output_file.write(output_line)

torch.save(model.state_dict(), 'subtaskB_xlmrobertabase.pth')